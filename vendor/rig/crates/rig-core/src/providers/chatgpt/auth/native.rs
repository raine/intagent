...
impl PlatformAuthenticator {
    ...
    fn write_auth_record(&self, record: &AuthRecord) -> Result<(), AuthError> {
        ...
        let temporary = temporary_auth_path(path)?;
...
fn reject_unsafe_destination(path: &Path) -> Result<(), std::io::Error> {
    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() || !metadata.is_file() => Err(
            std::io::Error::other("ChatGPT authentication cache must be a regular file"),
        ),
        Ok(_) => Ok(()),
...
fn temporary_auth_path(path: &Path) -> Result<PathBuf, std::io::Error> {
    ...
    let file_name = path
        ...
        .ok_or_else(|| {
            std::io::Error::new(
                ...
                "ChatGPT authentication cache has an invalid file name",
            )
        })?;
