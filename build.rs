...
fn main() {
    println!("cargo:rerun-if-env-changed=INTAGENT_DASHBOARD_DIR");

    let source_directory = env::var_os("INTAGENT_DASHBOARD_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
...
fn copy_asset(source: &Path, destination: &Path) {
    if !source.is_file() {
        panic!(
            "required dashboard asset {} is missing; run `npm ci --prefix web && npm run build --prefix web` or set INTAGENT_DASHBOARD_DIR to a directory containing app.js and app.css",
            source.display()
        );
    }
